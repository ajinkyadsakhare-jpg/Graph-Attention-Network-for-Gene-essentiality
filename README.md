...
Existing models predict this reasonably well. HELP (Granata et al.), short for Human Gene
Essentiality Labelling and Prediction, combines several kinds of information about each gene,
including a gene network. MAHI, from the Troyanskaya lab, does something similar. HELP was built
specifically for finding gene therapy targets.
...
But these models are precise. Are they rich? HELP splits its inputs into groups, and most of the
prediction comes from the group called CCcfs, around 3,000 features describing which compartment
of the cell each protein ends up in. The network contributes very little. So the network is in
the model, but it isn't doing the work.
...
| RESULTS.md | Each run in full: setup, numbers, and what each one rules out |
| `notebooks/` | The code |
...
- `notebooks/processing/` — turning raw DepMap, IID and HumanBase data into graphs and features
...
| 2 | GAT on IID | physical interaction network | pending re-run |
...
| BRCA1 | 1390 | ZNF350, BRIP1, BRAP, DCLRE1A |
...
At first this looked like the kidney signal being too sparse to learn. PAX8 did show up, which
is a known kidney lineage dependency, so something was there, but nearly all the values sat near
zero.
...
answers were compared. If it were noise the halves would disagree. They agree at 0.52.
...
Lung was also tried, since it has a larger dataset. About the same. A few parameter changes
...
The IID baseline runs at 2 attention heads against HumanBase's 4, because the larger graph
didn't fit in memory otherwise, and it has two edge features to HumanBase's one. Both are
differences between the two runs beyond the network itself.

## What's here and what isn't

Code and results are here. Data isn't, DepMap has its own terms and the files are large, so
`data/README.md` has links to download everything from the original sources. There's a
